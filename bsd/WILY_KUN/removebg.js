...
// Handler untuk command removebg
async function handleRemoveBgCommand(sock, msg) {
    try {
        ...
        // Cek apakah ada gambar di pesan saat ini (dengan caption .removebg)
...
💡 *Solusi:*
• Coba kirim ulang gambar dengan caption .removebg
• Pastikan gambar masih dapat dilihat
• Forward gambar lalu reply dengan .removebg
...
📝 *Cara penggunaan:*
• Kirim gambar dengan caption: ${config.bot.prefix}removebg
• Reply gambar dengan: ${config.bot.prefix}removebg

🖼️ *Contoh:*
• [kirim gambar] + caption "${config.bot.prefix}removebg"
• Reply gambar lalu ketik "${config.bot.prefix}removebg"
...
• Coba convert ke JPG/PNG terlebih dahulu`
        ...
        try {
            ...
            // Proses gambar melalui API siputzx
...
🤖 *WilyKun Bot - Advanced Image Tools*`
            ...
            if (error.message.includes('upload')) {
                ...
                errorMessage = '❌ Service remove background tidak tersedia';
            } else if (error.response?.status >= 500) {
                errorMessage = '❌ Server error, coba lagi nanti';
            } else if (error.code === 'ENOTFOUND') {
                errorMessage = '❌ Tidak dapat terhubung ke API siputzx';
            }
...
• Gambar dengan subjek yang jelas akan lebih baik

🌐 *API Status:*
• Provider: SiputZX API
• Status: ${error.response?.status || 'Connection Error'}`
        ...
        await sock.sendMessage(msg.key.remoteJid, {
            text: '❌ Terjadi kesalahan sistem saat memproses gambar remove background'
        }, { quoted: msg });
    }
}
